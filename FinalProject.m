...
Assignment: Final Project V0.3.0
...
set(startbut, 'Position', [0.15, 0.2, 0.15, 0.10], 'String', 'New Game');
set(startbut, 'Callback', {@cb_NewGameButton, titlescreen});
set(startbut, 'UserData', struct('choice', 0));
data = startbut.UserData;
choice = data.choice;
close('Infinichess');


%will be deleted later
tchoice = input('Do you want to open an existing game (y/n)?', 's'); 
while ~(strcmpi(tchoice, 'y') || strcmpi(tchoice, 'n'))
    fprintf('Invalid input.\n');
    tchoice = input('Do you want to open an existing game (y/n)?', 's')
end
if strcmpi(tchoice, 'y')
    choice = 2;
else
    choice = 1;
end
...
%I plan to replace these inputs with a GUI
if choice == 2
